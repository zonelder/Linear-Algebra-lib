...
namespace LibsUnitTest
{
	...
	public:
		...
		TEST_METHOD(createShortCuts)
		{
			...
			Assert::AreEqual(Vec3::One.ToString(), Vec3(1.0, 1.0f,1.0f).ToString(), MSG("up shortcut is not equal to (1,1)"));
		}
		...
		TEST_METHOD(VectorAlgebra)
		{
			Vec3 a = Vec3::One;
			Vec3 b = Vec3::Down;
			Assert::IsTrue((a + b) == Vec3(1.0f,0,1), MSG("(1,1)+(0,-1) != (1,0). operator's + work is not corrent"));
			Assert::IsTrue((-a) == Vec3(-1, -1,-1), MSG(" -(1,1) != (-1,-1).work of  unar minus operator is incorrent"));

			Assert::IsTrue((b - a) == Vec3(-1, -2,-1), MSG("(0,-1) - (1,1) != (-1,-2).work of  minus operator is incorrent"));

			Assert::IsTrue((2 * a) == Vec3(2, 2,2), MSG("work of LHS multiplication is incorrent"));
			Assert::IsTrue((a * 2) == Vec3(2, 2,2), MSG("work of RHS multiplication is incorrent"));
			Assert::IsTrue((2 * a) == (a * 2), MSG("RHS mult equal to LHS mult"));

			Assert::IsTrue((a / 2) == Vec3(0.5, 0.5,0.5), MSG("dividing vector by number is incorrent"));
		}
